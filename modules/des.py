...
class des(_baseDes):
    ...
    def crypt(self, data, crypt_type):
        ...
        if(not data):
            return ''
        if(len(data) % self.block_size != 0):
            if crypt_type == des.DECRYPT:    # Decryption must work on 8 byte
...
class Des(object):
    ...
    def __init__(self, psw, ivv, typ, size):
        ...
        if(size == 24):
            cla = triple_des(psw[2:size + 2], CBC, ivv)
        else:
            cla = des(psw[2:size + 2], CBC, ivv)
        if(boool):
...
def make_iv(size):
    ...
    @param integer size:        type of des [8,24]
    @return list
    '''
    ...
    for i in range(0, size):
        try:
            res = '%s%s' % (res, int.from_bytes(urandom(1), 'little'))
        except IndexError:
            res = '%s%s' % (res, i)
    if(len(res) > 8):
        res = res[0:8]
