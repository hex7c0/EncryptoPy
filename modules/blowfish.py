...
Modified by (CTR changes)     Ivan Voras (ivoras@gmail.com)
@license http://www.gnu.org/licenses/gpl.html GPL v2 License
...
BLOCK_SIZE = 8
KEY_LENGTH = 56
KEY_BITS = KEY_LENGTH * BLOCK_SIZE
...
class Blowfish(object):
    ...
    equivalently 8 and 56 bytes.
    The encryption and decryption functions operate
    ...
    def __init__(self, key):
        ...
        self.ctr_iv = self.ctr_cks = self.ctr_pos = None
    ...
    def encrypt(self, data):
        ...
        if(not len(data) == 8):
            raise Exception('invalid block length: %s' % len(data))
    ...
    def init_ctr(self, iv=0):
        '''
        Initializes CTR engine for encryption or decryption.
        '''

        assert struct.calcsize('Q') == BLOCK_SIZE
        self.ctr_iv = iv
        self._calc_ctr_buf()

    def _calc_ctr_buf(self):
    ...
    def encrypt_ctr(self, data):
        ...
        result = []
        for ch in data:
            result.append(chr(ord(ch) ^ self._next_ctr_byte()))
        return ''.join(result)

    def decrypt_ctr(self, data):
        '''
        Decrypts a string encrypted with encrypt_ctr() and
...
def strip_psw(psw):
    '''
    strip password if length ranging out from 8 to 56 bytes
...
def set_data(data):
    '''
    DEPRECATED
    change len of buffer to len of 8 byte
    @param byte data    file read binary
    '''
